...
class ContentScreener:
    ...
    def __init__(self):
        ...
        self._inappropriate_terms = [
            "violent", "kill", "murder", "blood", "death", "weapon", "gun", 
            "knife", "suicide", "drug", "alcohol", "naked", "nude", "sex",
            "explicit", "terror", "nightmare", "horror", "gruesome",
            # Additional terms would be included in a real implementation
        ]
        
        # Compile regex patterns for efficiency
        self._inappropriate_pattern = re.compile(
            r'\b(' + '|'.join(re.escape(term) for term in self._inappropriate_terms) + r')\b', 
            re.IGNORECASE
        )
        
        # Topics appropriate for children's stories
        self._appropriate_topics = [
            "friendship", "adventure", "animals", "family", "learning",
            "imagination", "nature", "kindness", "helping", "sharing",
            "courage", "creativity", "discovery", "growing up", "teamwork"
        ]
    ...
    def suggest_alternative(self, prompt: str) -> str:
        ...
        positive_themes = [
            ...
            if word in self._appropriate_topics
        ]
    ...
    def screen_generated_content(self, content: str) -> Dict[str, Any]:
        ...
        if matches:
            logger.warning(f"Inappropriate content detected in generated story")
            return {
