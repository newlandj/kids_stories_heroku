...
def mock_record_narration(pages):
    return [
        {"audio_url": f"https://mock-s3-bucket.amazonaws.com/audio/mock-narration-{i}.mp3", "page_index": i}
...
async def run_story_pipeline_test():
    ...
    if mock_mode:
        ...
        with mock.patch.object(FableFactory, 'weave_narrative', side_effect=mock_openai_generate_story), \
             mock.patch.object(FableFactory, '_identify_key_pages', side_effect=mock_identify_key_pages), \
             ...
             mock.patch("app.storage.upload_file_to_s3", side_effect=mock_upload_file_to_s3):
        ...
        with mock.patch("app.storage.upload_file_to_s3", side_effect=mock_upload_file_to_s3):
            factory = FableFactory()
            result = await factory.generate_story_package(test_prompt)
    else:
    ...
    assert "pages" in result and isinstance(result["pages"], list) and len(result["pages"]) > 0
